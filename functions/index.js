...
/**
 * Cleans and filters warnings generated by the AI model.
 * It strips common weather prefixes, filters out generic weather forecasts, and filters out general traffic.
 * It always preserves severe weather alerts that could impact access to the trailhead.
 */
export function cleanWarnings(warnings) {
  if (!warnings || !Array.isArray(warnings)) return [];

  const weatherPrefixRegex = /^(?:(?:\d+%\s*chance\s*of\s*(?:rain|precipitation|snow|showers|drizzle|storm|storms|weather)(?:\s*in\s*the\s*(?:region|area|forecast))?)|(?:chance\s*of\s*rain\s*(?:is\s*)?\d+%)|(?:weather\s*forecast\s*predicts\s*[^;:,.]+)|(?:with\s*a\s*\d+%\s*chance\s*of\s*(?:rain|precipitation|snow|showers|drizzle|storm|storms)(?:\s*in\s*the\s*(?:region|area))?)|(?:due\s*to\s*(?:a\s*)?\d+%\s*chance\s*of\s*(?:rain|precipitation|snow|showers|drizzle|storm|storms)))(?:[;:,.]\s*|\s+-\s*|\s+)/i;
  ...
  for (const warning of warnings) {
    ...
    const lowerOrig = w.toLowerCase();
    const isSevereWeatherAccess = lowerOrig.includes('storm') || 
                                  lowerOrig.includes('flood') || 
                                  lowerOrig.includes('blizzard') || 
                                  lowerOrig.includes('gale') || 
                                  lowerOrig.includes('high wind') || 
                                  lowerOrig.includes('heavy snow');

    const match = w.match(weatherPrefixRegex);
    if (match) {
      w = w.slice(match[0].length).trim();
      if (w) {
    ...
    if (!isSevereWeatherAccess) {
      const lowerClean = w.toLowerCase();

      const isGenericWeatherRadar = lowerClean.includes('weather radar') || 
                                    lowerClean.includes('local weather radar') || 
                                    lowerClean.includes('check local forecast') ||
                                    lowerClean.includes('check weather radar') ||
                                    lowerClean.includes('radar before departure');

      const isGenericRainWarning = (lowerClean.includes('chance of rain') || 
                                    lowerClean.includes('chance of precipitation') || 
                                    lowerClean.includes('rain is forecast') || 
                                    lowerClean.includes('rain in forecast') || 
                                    lowerClean.includes('expect rain') || 
                                    lowerClean.includes('rain is expected')) && 
                                   !lowerClean.includes('mud') && 
                                   !lowerClean.includes('footwear') && 
                                   !lowerClean.includes('trail') && 
                                   !lowerClean.includes('spikes') && 
                                   !lowerClean.includes('crossing') && 
                                   !lowerClean.includes('creek') &&
                                   !lowerClean.includes('river');

      const isGeneralWeatherStatement = (lowerClean.includes('weather forecast') || 
                                         lowerClean.includes('temperature highs') || 
                                         lowerClean.includes('highs near') || 
                                         lowerClean.includes('temps in the') || 
                                         lowerClean.includes('high temperature') ||
                                         lowerClean.includes('mild weather')) && 
                                        !lowerClean.includes('heat') && 
                                        !lowerClean.includes('dehydration') && 
                                        !lowerClean.includes('shade') && 
                                        !lowerClean.includes('water');

      const isGeneralTraffic = lowerClean.includes('expect moderate traffic') || 
                               lowerClean.includes('expect heavy traffic') || 
                               lowerClean.includes('traffic on') || 
                               lowerClean.includes('traffic delays') ||
                               lowerClean.includes('highway traffic');

      if (isGenericWeatherRadar || isGenericRainWarning || isGeneralWeatherStatement || isGeneralTraffic) {
        console.log(`Filtering out generic/contradicting warning: "${warning}" (cleaned: "${w}")`);
        continue;
      }
    }
...
export const searchAdventures = onCall({ 
  ...
  try {
    ...
    const enrichedPromises = activities.map(async (activity) => {
      ...
      // Clean and filter Gemini's warnings to avoid duplicates/contradictions
      const cleanedWarnings = cleanWarnings(activity.warnings);
