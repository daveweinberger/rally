...
function App() {
  const { search, reset, cancel, results, setResults, setGeneralExplanation, setGroundingMetadata, generalExplanation, noResultsExplanation, groundingMetadata, status, statusMessage, error, isLoading } = useAdventureSearch();
  ...
  const handleSearchSubmit = (constraints) => {
    ...
    try {
      ...
      console.warn('Failed to save constraints to sessionStorage:', e);
    }
    search(constraints);
  };
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', width: '100%' }}>
      ...
      <div className="gorp-container">
        ...
        <main id="main-content" style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          {isLoading && (
            <LoadingState status={status} statusMessage={statusMessage} constraints={activeConstraints} onCancel={cancel} />
          )}
              ...
              {results.length === 0 ? (
                <div className="glass-card flex-col align-center gap-md" style={{ padding: '2.5rem 2rem', textAlign: 'center', borderTop: '4px solid #e0a150' }}>
                  ...
                  <p style={{
                    ...
                    {noResultsExplanation || "We couldn't find any activities matching your exact preferences under the current weather forecast."}
                  </p>
